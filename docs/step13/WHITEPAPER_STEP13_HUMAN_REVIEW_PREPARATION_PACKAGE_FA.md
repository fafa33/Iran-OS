...
> **⚠️ جایگزین‌شده (پایه مخزن):** جدول‌های «پایه مخزن» این سند snapshot تاریخی در `f9d47da` (۴۹۹ تست) هستند و دیگر مرجع «وضعیت جاری» نیستند. بازبین‌ها پیش از شروع، پایه مخزن جاری را از `WHITEPAPER_STEP13_CURRENT_STATUS_CHECKPOINT_V4_FA.md` بردارند. محتوای آماده‌سازی review بازنویسی نشده و معتبر است.
