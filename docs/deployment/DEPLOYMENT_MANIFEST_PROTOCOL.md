...
>
> **به‌روزرسانی (نسخه ۱.۳.۰):** اسکریپت‌های `deploy/` اکنون برای **۶ از ۲۵ قرارداد** (مسیر اصلی پولی/پشتوانه: `IranOS_Kernel`، `Treasury`، `SovereignWealthFund`، `PahlaviToken`، `API3Oracle`، `RecognizedReserveBacking`) در مخزن موجودند — به بخش ۱۱ و `deploy/README.md` مراجعه کنید. هیچ dry-run روی testnet، هش bytecode، یا تخمین gas مستند نشده است. اسکریپت برای ۱۹ قرارداد باقی‌مانده (رفاه، قضایی، حاکمیتی، بازپس‌گیری) هنوز ایجاد نشده است. `STEP9-BLOCK-005` همچنان **OPEN/PENDING** است — این به‌روزرسانی بسته شدن آن را ادعا نمی‌کند.
...
□ اسکریپت‌های deploy/ با ترتیب مشخص موجود هستند — برای ۶ از ۲۵ قرارداد (مسیر اصلی پولی/پشتوانه) برقرار است؛ برای ۱۹ قرارداد باقی‌مانده هنوز نه (`deploy/README.md`)
...
**وضعیت واقعی (نسخه ۱.۳.۰):** اسکریپت‌های بالا برای پوشش کامل ۲۵/۲۵ هنوز طرح (پیشنهاد نام‌گذاری فایل) هستند، نه پیاده‌سازی. آنچه **واقعاً در مخزن پیاده‌سازی شده** ساختاری متفاوت با نام‌گذاری متفاوت است که تنها ۶ قرارداد مسیر اصلی پولی/پشتوانه را پوشش می‌دهد:
...
TriggerProtocol، AssetFreeze، و ۱۷ قرارداد دیگر (رفاه، قضایی، حاکمیتی) در این پیاده‌سازی پوشش داده نشده‌اند — `deploy/README.md` فهرست کامل را مستند می‌کند. هیچ dry-run روی testnet/mainnet، هش bytecode، یا تخمین gas ثبت نشده است. `STEP9-BLOCK-005` همچنان یک باقی‌مانده G-11 فنی است.
...
- اسکریپت‌های `deploy/` برای ۶ از ۲۵ قرارداد (مسیر اصلی پولی/پشتوانه) در مخزن پیاده‌سازی شده‌اند (`deploy/README.md`)؛ برای ۱۹ قرارداد باقی‌مانده (رفاه، قضایی، حاکمیتی، بازپس‌گیری، TriggerProtocol، AssetFreeze) هنوز ایجاد نشده‌اند — باقی‌مانده G-11 فنی
