...
| GAP-008 | VelocityFee | Added one deterministic dormant-liquidity boundary test covering unauthorized protected actions, state-neutral failed paths, explicit oracle fee application, tier threshold fee calculation, staking exemption, and below-threshold non-effect. | `test/11_Velocity_Fee.test.js` | Unauthorized callers cannot register accounts, record activity, apply fees, or toggle staking; failed attempts preserve account status and collected fees; dormant tier-2 balance receives the expected fee only through the authorized oracle path; staking and below-threshold accounts remain fee-neutral. | Partial remediation evidence added; formal token-balance dependency and fee-accounting proof targets remain open. |
| GAP-009 | Fargard7PolicyAdapter | Added one deterministic adapter-boundary test covering unauthorized recommendation/config/review rejection, state-neutral failed paths, stale/invalid signal rejection, fresh recommendation creation, reviewer approve/reject/expire paths, duplicate/late review rejection, and downstream non-execution. | `test/26_Step7_PolicyLayer.test.js` | Unauthorized adapter actions preserve config, recommendation count, and review state; fresh PriceOracle quorum creates non-executable recommendations; reviewer approval/rejection/expiration updates only adapter review metadata; stale or invalid oracle signals cannot create new recommendations; BaseIncome, BudgetAllocation, VelocityFee, ProductionOracle, and Provincial state remain unchanged. | Partial remediation evidence added; formal downstream non-interference and execution-path separation proof targets remain open. |
