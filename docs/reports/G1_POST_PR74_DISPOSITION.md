...
1. **ZK circuit (Circom or Noir):** A circuit proving that the voter knows a secret `s` such that `hash(s, caseId, nullifier) == commitment`, without revealing `s`. The nullifier must be stable per juror/case (e.g., `hash(secret, caseId)`) to provide one-time-use protection — it must not include `isGuilty` in its preimage (see item 4).
...
4. **Nullifier design:** The nullifier must be stable per juror/case — e.g., `hash(secret, caseId)` — not per juror/case/vote. Including `isGuilty` in the nullifier preimage would create two distinct unspent nullifiers per juror per case (one guilty, one not-guilty), allowing a juror to cast two votes and defeat double-submission prevention. Vote-flipping protection (preventing a valid proof from being submitted with the opposite `isGuilty` value) is already provided by including `isGuilty` as a public input in item 3: `verifier.verifyProof(proof, [commitment, caseId, isGuilty, nullifier])` binds the proof cryptographically to the specific vote submitted. The existing `usedCommitments[commitment]` mapping in `JurySelection.sol` currently fulfills the same role as a nullifier registry; a future implementation must preserve this deduplication guarantee, whether via commitment-keyed storage or a separate nullifier registry.
