...
| GAP-MEX-04: oracle data freshness at call boundary | **CLOSED** — two-gate freshness barrier implemented on `API3Oracle.syncReserves` (PR #85): Gate A requires PAH_USD_KEY within MAX_DATA_AGE; Gate B rate-limits reserve sync to one per MAX_DATA_AGE window. **Gate B constraint on breach recovery:** if a sync has already occurred in the current MAX_DATA_AGE window, a corrective `syncReserves(higher_value)` call is blocked until `lastReservesSyncTimestamp + MAX_DATA_AGE`. Burn-based recovery (`PahlaviToken.burn`) is the only in-window correction path for a breach caused by an earlier wrong-value sync. |
