...
### Audit and Proof Prerequisite Matrix

| Area | Prerequisite | Evidence | Owner | Blocker if missing | Status |
| --- | --- | --- | --- | --- | --- |
| External audit | Independent audit of Step-8 critical contract surfaces, authority boundaries, and Step-9 production doctrine. | Final audit report, audit scope, finding register, severity triage, remediation links, accepted-risk notes, and reviewer signoff. | External audit coordinator. | Production readiness cannot be claimed; open critical/high findings block release signoff. | Not complete. |
| Formal verification | High-priority formal verification targets are proven or explicitly accepted as unresolved risk. | Proof artifact index, tool output, assumptions file, target-to-contract mapping, counterexample notes, and formal reviewer signoff. | Formal methods owner. | Formal verification cannot be claimed; unresolved high-priority properties block production readiness unless formally accepted as risk. | Not complete. |
| Invariant proofs | Safety invariants for Kernel role reachability, Pahlavi supply/reserve constraints, SWF accounting, BudgetAllocation spend bounds, and adapter non-interference are specified and reviewed. | Invariant specification, harness or proof target, mapped tests, proof output or risk acceptance, and reviewer notes. | Engineering maintainer with formal verification reviewer. | Test-backed evidence remains insufficient for production proof claims. | Pending. |
| Oracle integrity | PriceOracle/API3/ProductionOracle assumptions for quorum, staleness, invalidation, deviation handling, feeder liveness, and signal-only boundaries are audited. | Oracle operations runbook, feeder registry, freshness model, deviation review procedure, invalidation evidence, and audit disposition. | Oracle operations lead. | Oracle signals may be under-specified; any oracle-to-policy execution ambiguity blocks production readiness. | Pending. |
| Role custody | Production role custody, key management, signer rotation, compromised-key response, and multi-sig evidence requirements are complete. | Role custody table, signer registry, quorum rules, rotation records, onboarding/offboarding procedures, and custody rehearsal notes. | Governance operations lead. | Critical authority custody remains undefined; single-person custody or missing quorum blocks release signoff. | Pending. |
| Deployment scripts | Deployment procedure, artifacts, constructor arguments, dependency wiring, role assignment, and post-deploy verification are reproducible and reviewed. | Deployment manifest, artifact hashes, script review, address book, constructor argument record, role assignment log, and dry-run output. | Deployment coordinator. | Deployment candidate cannot be reproduced or verified; unknown deployer/script behavior blocks release. | Pending. |
