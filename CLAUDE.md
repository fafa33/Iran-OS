...
│   │   └── PahlaviToken.so              # National currency token
│   ├── governance/
...
**`PahlaviToken.so`** — National currency token (note: file extension is `.so`, not `.sol` — check before editing).
...
| Monetary | `monetary-protocol-fa.md` | `SovereignWealthFund.sol`, `PahlaviToken.so` |
...
- The `PahlaviToken.so` file has a `.so` extension instead of `.sol` — this is not a compiled shared library; it is a Solidity source file with a typo in the extension. Treat it as Solidity.
