...
**Known structural limitation — squash-merge commit SHA:** under this repository's squash-merge convention, the exact merge commit SHA landing on `main` is assigned by GitHub at merge time and is not knowable before merge. A PR satisfies this step by (a) setting "Latest Merged PR" to its own PR number, (b) recording the literal, current **Reviewed Head SHA** value (the actual commit hash open for review at the time of writing — not merely a note that a future check is needed) together with the best-available review timestamp (the commit's own authored timestamp if no separate review-timestamp mechanism exists) and an explicit "Pending final merge SHA" placeholder, and (c) not leaving the file pointing at an older, now-superseded PR or commit as if it were still current. After merge, the final merged SHA replaces the pending placeholder while the previously-recorded Reviewed Head SHA is preserved in the surrounding text, so a reader can always trace which exact commit satisfied this step's pre-merge review. This is a disclosed limitation, not a certainty-language violation — no claim is made about a SHA that does not yet exist.
