...
### Fixed — Canonical Checkpoint post-merge sync (Step 13 follow-through)
- `docs/governance/CANONICAL_CHECKPOINT.md` — PR #122 merged as `8033b0b9dabd35e15281fe06165c4a2eb53ccf5c`; per Step 13's own disclosed squash-merge-SHA limitation, the pre-merge version of this file could only record PR #122 as "Pending" with a pointer to confirm the true SHA after merge. This commit performs that confirmation (`git log origin/main -1` → `8033b0b9d...`) and updates Latest Merged PR, Latest Commit, Current Merged Baseline, and the last-updated timestamp accordingly. Re-verified all other tracked fields (test count, deployment coverage, roadmap position, open residuals, latest LL ID) against current `main` — none had drifted; no field beyond the merge-identity fields required a change
- Governance-registry change only; no changes to `contracts/`, `deploy/*.js`, or test behavior. 759 passing (unchanged)
### Added — Governance: Governance Minimalism Principle (permanent rule, Step 15, LL-031)
- `CLAUDE.md` — the existing Extension rule (`## Canonical Development Workflow (Mandatory)`) grown into the full Governance Minimalism Principle: a new permanent governance rule, Lesson Learned, workflow step, merge gate, checklist item, Evidence Block, protocol, or permanent document may be added only if it satisfies all seven of: closes a demonstrated engineering failure, cannot be solved by extending an existing rule, does not duplicate an existing responsibility, does not create parallel terminology, has a clearly defined owner, has a measurable verification method, and permanently reduces future engineering risk. New `#### Step 15 — Governance Minimalism Review` added to `### PR Preflight Standard (Mandatory)`: the 7-criterion gate plus a required Governance Duplication Review (can an existing rule/LL/checklist/workflow stage absorb this instead?) — if any answer is YES, extend the existing artifact rather than create a new one. `## Canonical Development Workflow (Mandatory)` Stage 11 extended once more to reference Step 15 (no new Stage added)
- `docs/governance/REVIEWER_LESSONS_LEARNED.md` — added `LL-031`, root cause: "Governance systems naturally accumulate duplicate rules over time unless simplicity is explicitly protected"; includes the Governance Duplication Review applied to this very entry (dogfooding) — extending the Extension rule and the existing PR-template checklist/Evidence Block section rather than creating new top-level structure; Maintenance Rule updated "Steps 1–14" → "Steps 1–15"; footer updated to "LL-001 through LL-031"
- `.github/pull_request_template.md` — Step 15 checklist item and Evidence Block sub-block added to the *existing* `## Governance Preflight Synchronization` section (deliberately no new top-level PR-template section, per the Duplication Review's own finding that the existing section could absorb it)
- `docs/governance/CANONICAL_CHECKPOINT.md` — Applicable Governance Version and Latest Lesson Learned ID fields updated to Steps 1–15 / LL-031
...
### Added — Governance: Governance Synchronization Review (permanent rule, Step 14, LL-030)
- `CLAUDE.md` — new `#### Step 14 — Governance Synchronization Review` added to `### PR Preflight Standard (Mandatory)`: for any PR affecting governance state, engineering workflow, deployment workflow, security workflow, roadmap state, production readiness, or a permanent engineering rule, the author must independently determine whether each of the three authoritative governance artifacts (Canonical Checkpoint, Lesson Learned Registry, `CHANGELOG.md`) requires an update, and record `Updated` or `No update required` (with reason) for each in the PR Evidence Block — never inferring one artifact's answer from another's. `## Canonical Development Workflow (Mandatory)` Stage 11 and the Single Source of Truth rule extended to name all three artifacts and their distinct, non-duplicating responsibilities (previously the rule named only the Checkpoint)
- `docs/governance/REVIEWER_LESSONS_LEARNED.md` — added `LL-030`, root cause: "Governance artifacts remained individually correct but synchronization between authoritative governance documents was not explicitly verified"; Maintenance Rule updated "Steps 1–13" → "Steps 1–14"; footer updated to "LL-001 through LL-030"
- `.github/pull_request_template.md` — new `## Governance Synchronization Review` section (mirrors the existing per-concern section pattern), a Step 14 checklist line, and a Step 14 Evidence Block sub-block
- `docs/governance/CANONICAL_CHECKPOINT.md` — Applicable Governance Version and Latest Lesson Learned ID fields updated to Steps 1–14 / LL-030; Single Source of Truth Policy extended with a pointer to the new three-artifact table in Step 14 (not duplicated into this file)
- This entry is itself an example of the Step 14 review it introduces: all three artifacts (this changelog, the Lesson Learned registry, and the Canonical Checkpoint) are updated in the same commit
- Governance-registry and documentation changes only; no changes to `contracts/`, `deploy/*.js`, or test behavior. 759 passing (unchanged)
